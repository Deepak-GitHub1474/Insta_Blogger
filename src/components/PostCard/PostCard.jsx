...
import { addComment, deleteBlog, likeBlog } from "@/lib/action";
...
const PostCard = ({posts, users, comments, user}) => {
    ...
    const [likeState, likeFormAction] = useFormState(likeBlog, undefined);
      ...
      {posts.map(blog => (
            <div className="flex items-center justify-center flex-col" key={blog._id}>
                <div className=" max-w-[550px] w-[90vw] h-[500px] bg-gray-700 text-white my-4 p-4 pt-0 rounded-lg relative overflow-hidden shadow-[0_5px_5px_white] hover:shadow-[0_5px_5px_#0000ff] transition-all duration-300">
                    ...
                    <div className="mt-8">
                        <p className="font-light overflow-y-auto max-h-32 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">{blog?.description}</p>
                    </div>
                    <div className="mt-2 flex items-center justify-between gap-8">
                        <div className="flex flex-col items-center text-gray-400 w-1/2">
                            ...
                            <span className="max-w-48 whitespace-nowrap text-ellipsis overflow-hidden text-sm cursor-pointer hover:text-gray-300">{blog.likesCount} likes</span>
                            ...
                            <div className=" absolute left-0 top-0 z-50">
                                <div className=" bg-white text-black max-w-[550px] w-[90vw] h-[500px] overflow-hidden relative">
                                    <div className="absolute right-0 w-8 h-8 bg-blue-600 text-white font-bold 
                                                    text-center rounded-full pt-1 cursor-pointer hover:bg-red-600" onClick={() => openCommentBox(blog._id)}>
                                        <span>X</span>
                                    </div>
                                    <section>
                                            ...
                                            {blog?._id === comment?.postId && 
                                                <div className="flex gap-4 m-3" key={comment._id}>
                                                    <div className="relative w-[2rem] h-[2rem] rounded-full overflow-hidden border-[0.5px] border-gray-400 mt-1">
                                                        <img src={comment?.img ? comment?.img : "/avatar.png"} fill alt="post-cover" className="object-cover"/>
                                                    </div> 
                                                    <div className="flex flex-col gap-2 text-sm">
                                                        <div className="flex items-center gap-2">
                                                            <p className=" font-semibold">{comment?.username}</p>
                                                            <p className="font-semibold text-gray-500 text-xs">{comment?.createdAt.toString().slice(4, 16)}</p>
                                                        </div>
                                    ...
                                    <section className="flex flex-col gap-1">
                                        ...
                                        <div className="flex items-center justify-between text-xl px-1">
                                            <span className="cursor-pointer hover:scale-[1.3] transition-all duration-300" onClick={() => setCommentText(commentText+"❤️")}>❤️</span>
                                            <span className="cursor-pointer hover:scale-[1.3] transition-all duration-300" onClick={() => setCommentText(commentText+"😂")}>😂</span>
                                            <span className="cursor-pointer hover:scale-[1.3] transition-all duration-300" onClick={() => setCommentText(commentText+"😮")}>😮</span>
                                            <span className="cursor-pointer hover:scale-[1.3] transition-all duration-300" onClick={() => setCommentText(commentText+"😍")}>😍</span>
                                            <span className="cursor-pointer hover:scale-[1.3] transition-all duration-300" onClick={() => setCommentText(commentText+"🎉")}>🎉</span>
                                            <span className="cursor-pointer hover:scale-[1.3] transition-all duration-300" onClick={() => setCommentText(commentText+"😭")}>😭</span>
                                        ...
                                        <div className="flex items-center gap-2 p-1 text-sm">
                                            ...
                                            <form action={commentFormAction}>
                                                ...
                                                <input 
                                                    ...
                                                    className="p-2 outline-none sm:w-[27rem] w-[60vw]"
                                                />
                                                {commentText.length > 0 && <button className="ml-1 p-2 text-blue-600 font-semibold hover:text-red-600 transition-all duration-300">post</button>}
                                            </form>
                                        </div>
                                    </section>
